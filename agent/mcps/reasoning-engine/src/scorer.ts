 ...
 */
export function computeScore(scores: CriteriaScores, survived: number, total: number): number {
  const raw =
    WEIGHTS.logicalConsistency    * clamp(scores.logicalConsistency) +
 ...
 */
export function countSurvivedAssumptions(
  assumptions: Assumption[],
  attacks: Attack[]
): { survived: number; killed: number; total: number } {
