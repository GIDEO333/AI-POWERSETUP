...
LIST_SKILLS_TOOL = {
    "name": "list_skills",
    "description": "List all 42+ agent skills. Categories: Dev (api-design, create-project, database-query, refactor-code, write-tests), Debug (debug-code, optimize-performance), Ops (docker-security-audit, docker-setup, git-workflow, headless-cli-wrapper, powersetup-healthcheck, safe-push, safe-rollback, sandbox-executor-setup), AI-Orchestration (agent-constraint-schema, claude-agent-teams, multi-agent-orchestrator, multi-session-architect), QA (ai-system-evaluator, code-review, llm-eval-arena, project-audit, query-scorecard, source-truth-check), Memory (recall-memory, session-export, session-memory), Format (brainstorm-refiner, normalize-input), Research (deep-research, query-requestout), Meta (forge-to-claude, gemini-rules-toggle, meta-levelup, skill-creator, skills-hunter-session, token-budget, workflow-guide), App-Specific (nanoclaw-login). Call for full catalog.",
    "inputSchema": {
        "type": "object",
        "properties": {}
    }
